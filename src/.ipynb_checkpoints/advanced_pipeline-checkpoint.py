    ...
    def _persist_results(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            if not table_exists:
                ...
                # Table existante : suppression du mois en cours puis insertion
                self.logger.info(f"Table existante - Suppression des données pour MOIS_ANNEE={mois_annee_actuel}")

                # Suppression des anciennes données du mois
                delete_query = f"""
                ...
                try:
                    ...
                    self.logger.info(f"Données du mois {mois_annee_actuel} supprimées")
                except Exception as e:
                    self.logger.warning(f"Impossible de supprimer (table peut-être vide) : {e}")

                # Insertion des nouvelles données
                self.logger.info(f"Insertion de {len(df_to_save):,} nouvelles lignes")
                df_spark.write.mode("append").saveAsTable(output_table)
                self.logger.info("Insertion terminée avec succès")
